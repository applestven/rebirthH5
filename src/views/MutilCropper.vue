<template lang="">
    <div class="taleft fs12 ">
        <div v-if="!loading.flag">
           ...
           <div >
                <div class="cblue" @click="changeDialog('1')">
            ...
            <div style="width:100% ;display:flex;justify-content:space-between;flex-wrap: wrap" class="mt15">
                <div style="width:46%" v-for="(item , i) in groudList" class="mt15">
                            <div @click="clickGroud(i)">
            ...
            <van-dialog v-model="show"   width="95%" style="height:auto;overflow-y:auto;padding:5px" :lockScroll="false" @cancel="cancel" @confirm="confirm"> 
                ...
                <div style="width:100%;height:300px;">
                    <vueCropper
                        :info="false"
                        :class="{'a':$route.query.category=='圆形贴纸','b':$route.query.category=='圆形贴纸'}"
                        :fixedBox="true"
                        @realTime="realTime"
                        :autoCrop="true"
                        :autoCropWidth="queryData.width"
                ...
                <div style="height:250px;">
                        <div style="width:90%;height:200px;padding:10px;position: relative;display:flex;justify-content:center">
                            <div :style="{border:'1px dashed grey', width:queryData.width - 4+'px', height:queryData.height -4 +'px',position: 'absolute',top:'11px',borderRadius:$route.query.category == '圆形贴纸' ?'50%':''}"></div>
        ...
        <div v-if="loading.flag">
            ...
            <div class="cblue">
                <!-- <div>关于色差问题</div> <br> -->
                <div @click="changeDialog('4')">
                    <div v-html="noticeConfig[3].title" ></div> <br>
                </div>
                
                   
                <!-- <div>关于图片上传与售后问题</div><br> -->
                <div @click="changeDialog('5')">
                    <div v-html="noticeConfig[4].title" ></div><br>
                </div>
                
                    
                <div @click="changeDialog('6')">
                    <div v-html="noticeConfig[5].title" > 
                </div>
                    <!-- 关于图片裁切误差:
...
import { drawAndClipImage } from "../utils/circleImge"
export default {
    name: "Cropper",
    data() {
        return {
    ...
    methods: {
        ...
        async uploadImg() { // 上传图片
            ...
            this.groudList.forEach((item,index) => {
                if (item.frontList.length) {
                    // var renameReportFile = new File([item.frontList[0].file], '2-30.jpg', {type: item.frontList[0].file.type});
                    // 重命名
                    var renameReportFile = new File([item.frontList[0].file], index+1+'-'+item.number+'.jpg', {type: item.frontList[0].file.type});
                    fileList.push(renameReportFile)
        ...
        async confirm() {// 截图确定 
            ...
            let file = base64ImgtoFile(changedpi.changeDpiDataUrl(this.previewStyle.url, 300))
            // console.log('@file',file)
            if (this.$route.query.category == "圆形贴纸") {
                this.$set(this.groudList[this.loacation[0]].frontList, 0, await drawAndClipImage(file, "file.jpeg"))
            } else {
        ...
        async afterRead(data) { // 文件上传之后的处理  弃
            ...
            console.log('@图片规格需要的宽高', Number(this.queryData.width) * this.coefficient, Number(this.queryData.height) * this.coefficient)
            if (Number(this.queryData.width) * this.coefficient < wh.width || Number(this.queryData.height) * this.coefficient < wh.height) { // 图片需要大小 < 图片实际大小
                // console.log("")
        ...
        async afterReadRest(data) {
            ...
            if (Object.prototype.toString.call(imgOb) == '[object Object]') { // 单张图片上传 
                ...
                await this.getCropperImg(data)
                // setTimeout(() => { //延时关闭剪切板
                //     this.delCropperFlag = false 
                //     console.log("@关闭剪切板")
                // }, 1500);
                this.overlayshowFlag = false
                if (this.$route.query.category == "圆形贴纸") { // 其他两项不需要特殊处理 
                    let file = this.groudList[this.groudList.length - 1].frontList[0].file
                    this.$set(this.groudList[this.groudList.length - 1].frontList, 0, await drawAndClipImage(file, "file.jpeg"))
                }
                ...
                for (let i = 0; i < index; i++) {
                    let wh = await getImgInfo(data[i].content)
                    if (realWH.width > wh.width || realWH.height > wh.height) {
                        console.log('所选图片分辨率过小,请重新选择');
                        Toast("所选图片分辨率过小,请重新选择")
                        this.overlayshowFlag = false
                        return;
                    }
                ...
                //        await this.getCropperImg(imgOb[index])
                //         index -=1
                //     }else{// 裁剪失败 继续上一张 
                ...
                for (const item in imgOb) {
                    this.isoption.img = imgOb[item].content
                    await this.getCropperImg(imgOb[item]) // 处理照片 需要时间 
                    if (this.$route.query.category == "圆形贴纸") { // 其他两项不需要特殊处理 
                        // this.groudList[this.groudList.length-1].frontList = await 
                        let file = this.groudList[this.groudList.length - 1].frontList[0].file

                        this.$set(this.groudList[this.groudList.length - 1].frontList, 0, await drawAndClipImage(file, "file.jpeg"))
                        // this.groudList[this.groudList.length - 1].frontList[0].file =  base64ImgtoFile(changedpi.changeDpiDataUrl(this.groudList[this.groudList.length - 1].frontList[0].content, 300))
                    }
                }
                this.overlayshowFlag = false
                this.delCropperFlag = false
        ...
        getCropperImg(data) { // 辅助处理照片 ，需要轮询判断是否加载完成  加载完成 即完成reslove
            ...
            return new Promise((resolve, reject) => {
                let inter = setInterval(() => {
                    ...
                    this.$refs.iscropper.getCropData((cropData) => {
                        ...
                        this.isoption.img = ''
                        this.groudList.push({
                            frontList: [{ content: changedpi.changeDpiDataUrl(cropData, 300), file: base64ImgtoFile(changedpi.changeDpiDataUrl(cropData, 300)),imageFit:'contain' }],
                            originList: [{ ...data }],
                            number: 1
                        })
    ...
    async created() {
        ...
        if (this.$route.query.category == "圆形贴纸") {
            this.previewStyle.borderRadius = '50%';
        }
    }
}
</script>
...
.a ::v-deep .cropper-view-box {
    border-radius: 50%;
}

.b ::v-deep .cropper-face {
    border-radius: 50%;
}
