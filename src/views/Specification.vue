<template lang="">
    <div>
        ...
        <!-- 注意事项  文字配置-->
        <div v-html="noticeText" class="fs12 taleft " style=" line-height:1.3rem"></div>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        upImage(){
            if( this.inputVal.width >500 ||  this.inputVal.height >500 ){
                Toast("尺寸填写过大") 
                return ;
            }
            if(this.radio !="" && this.radio2 != "" && this.inputVal.width !=""){
                ...
                if(this.radio2 == '双面'){
                    this.$router.push({
                        path:'/cropper',
                        query:{ orderNumber:this.$route.query.orderNumber, category:this.radio , amount:this.radio2 ,width:this.inputVal.width,height:this.inputVal.height,craft:this.selectVal+this.customIpu }})
                }else{
                    this.$router.push({
                        path:'/mutilCropper',
                        query:{ orderNumber:this.$route.query.orderNumber,category:this.radio , amount:this.radio2 ,width:this.inputVal.width,height:this.inputVal.height?this.inputVal.height:this.inputVal.width,craft:this.selectVal+this.customIpu }})
                }
            }else{
